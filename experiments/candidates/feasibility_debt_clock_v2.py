"""Experiment-owned feasibility-debt clock v2 candidate.
...
class FeasibilityDebtBatchedRestartAdamV2(OptimizationAlgorithm):
    ...
    def _validate_feasibility_debt_aux(aux, population_size: int) -> None:
        ...
        power_extra = set(aux["power_values"]).difference(power_required)
        if power_extra:
            raise ValueError(
                f"unexpected power leaves: {sorted(power_extra)}"
            )
    ...
    def optimize(
        ...
        **kwargs,
        ...
        best_post_feasibility_total_loss = jnp.full(
            (population_size,), jnp.inf
        )
        ...
        while not obj.budget_exceeded:
            ...
            if progress_mode == "total_loss":
                ...
                ever_feasible = ever_feasible | first_feasible
                best_infeasible_debt = jnp.where(
                    pre_feasibility_improved, debt, best_infeasible_debt
                )
                post_feasibility_best_update = (
                    first_feasible | post_feasibility_improved
                )
                best_post_feasibility_total_loss = jnp.where(
                    post_feasibility_best_update,
                    losses,
                    best_post_feasibility_total_loss,
                )
                progress_value = jnp.where(ever_feasible, losses, debt)
            ...
            if (
                ...
                treatment_event_after = {
                    **treatment_event_before,
                    ...
                    "progress_best_post_feasibility_total_loss_after": (
                        best_post_feasibility_total_loss
                    ),
                }
